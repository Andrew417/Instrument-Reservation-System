...
export const instruments = pgTable(
  ...
  (table) => [
    check('booking_mode_check', sql`${table.bookingMode} IN ('manual', 'instant')`),
  ]
);
...
export const reservationSeries = pgTable(
  ...
  (table) => [
    check('pattern_type_check', sql`${table.patternType} IN ('weekly', 'custom')`),
  ]
);
...
export const reservations = pgTable(
  ...
  {
    id: uuid('id').primaryKey().defaultRandom(),
    seriesId: uuid('series_id').references(() => reservationSeries.id, { onDelete: 'set null' }),
    userId: uuid('user_id').references(() => users.id, { onDelete: 'set null' }),
    adminId: uuid('admin_id').references(() => admins.id, { onDelete: 'set null' }),
    instrumentId: uuid('instrument_id')
      .references(() => instruments.id, { onDelete: 'cascade' })
  ...
  (table) => [
    check('reservation_type_check', sql`${table.reservationType} IN ('in_church', 'outside_church')`),
    check(
      'status_check',
...
export const reservationSeriesRelations = relations(reservationSeries, ({ one, many }) => ({
  user: one(users, { fields: [reservationSeries.userId], references: [users.id] }),
  admin: one(admins, { fields: [reservationSeries.adminId], references: [admins.id] }),
  instrument: one(instruments, { fields: [reservationSeries.instrumentId], references: [instruments.id] }),
  reservations: many(reservations),
}));

export const reservationsRelations = relations(reservations, ({ one, many }) => ({
  series: one(reservationSeries, { fields: [reservations.seriesId], references: [reservationSeries.id] }),
  user: one(users, { fields: [reservations.userId], references: [users.id] }),
  admin: one(admins, { fields: [reservations.adminId], references: [admins.id] }),
  instrument: one(instruments, { fields: [reservations.instrumentId], references: [instruments.id] }),
  messages: many(messages),
}));
...
export const trustedStatusAuditLogRelations = relations(trustedStatusAuditLog, ({ one }) => ({
  targetUser: one(users, { fields: [trustedStatusAuditLog.targetUserId], references: [users.id] }),
  grantedByAdmin: one(admins, { fields: [trustedStatusAuditLog.grantedByAdminId], references: [admins.id] }),
}));
