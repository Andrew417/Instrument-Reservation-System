...
export async function createSession(
  ...
  if (isUser) {
    ...
    if (!u || !u.isActive || u.approvalStatus !== "approved") {
      throw new Error(
        "Cannot create session for inactive or unapproved account",
      );
    }
    userDetails = {
      id: u.id,
...
export async function validateSession(
  token: string,
): Promise<{ valid: boolean; session?: ActiveSession; error?: string }> {
  if (!token) {
  ...
  const timeSinceLastActivity =
    now.getTime() - new Date(s.lastActiveAt).getTime();
...
export async function requireAuth(
  req: Request & { user?: SessionUser; session?: ActiveSession },
  res: Response,
  next: NextFunction,
): Promise<void> {
  ...
  if (!valid || !session) {
    res
      .status(401)
      .json({ error: error || "Invalid or expired session", isExpired: true });
    return;
  }
...
export async function withUserContext<T>(
  ...
  try {
    ...
    await client.query(`SELECT set_config('app.current_role', $2, true)`, [
      user.role,
    ]);
    await client.query(`SELECT set_config('app.is_admin', $3, true)`, [
      isAdmin ? "true" : "false",
    ]);
