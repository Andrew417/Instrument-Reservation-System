...
async function requireSuperAdminAuth(
  ...
  try {
    ...
    if (session.role !== "super_admin" && !session.user?.isSuperAdmin) {
      res.status(403).json({
        success: false,
        error: "Restricted Action. Super Administrator privileges required.",
      });
      return;
    }
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const todayStr = (req.query.date as string) || getCairoDateString();
      ...
      const totalInstruments = Number(
        (instCountRes as any).rows?.[0]?.count || 0,
      );
      ...
      const pendingRequests = Number(
        (pendingCountRes as any).rows?.[0]?.count || 0,
      );
      ...
      WHERE status IN ('approved', 'ongoing')
        AND (lower(time_range) AT TIME ZONE 'Africa/Cairo')::date = ${todayStr}::date
    `);
      const todayReservations = Number(
        (todayCountRes as any).rows?.[0]?.count || 0,
      );
      ...
      const activeUsers = Number(
        (membersCountRes as any).rows?.[0]?.count || 0,
      );
      ...
      const pendingUserApprovals = Number(
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const adminId =
        (req as any).adminSession?.adminId || (req as any).adminUser?.id || "";
      const result = await adminRejectReservation(
        id,
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const adminId =
        (req as any).adminSession?.adminId || (req as any).adminUser?.id || "";
      const result = await adminBulkReject(
        ids,
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (list.length === 0) {
        res.status(400).json({
          success: false,
          error: "No reservation IDs provided for bulk deletion.",
        });
        return;
      }
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.json({
        success: true,
        ...result,
        message: "All pending series occurrences approved.",
      });
    } catch (err: any) {
      res.status(400).json({ success: false, error: err.message });
    }
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const adminId =
        (req as any).adminSession?.adminId || (req as any).adminUser?.id || "";
      const result = await adminRejectSeries(
        seriesId,
        reason || "Series rejected by administrator.",
        adminId,
      );
      res.json({
        success: true,
        ...result,
        message: "All series occurrences rejected.",
      });
    } catch (err: any) {
      res.status(400).json({ success: false, error: err.message });
    }
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!name || !name.trim()) {
        res
          .status(400)
          .json({ success: false, error: "Instrument name is required." });
        return;
      }
      if (!type) {
        res.status(400).json({
          success: false,
          error: "Instrument type category is required.",
        });
        return;
      }

      const fee =
        outsideFeePerDay !== undefined &&
        outsideFeePerDay !== null &&
        outsideFeePerDay !== ""
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!finalPhoto) {
        res
          .status(400)
          .json({ success: false, error: "No image data or URL provided." });
        return;
      }
      res.json({
        success: true,
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const [existingAdmin] = await db
        .select({ id: admins.id })
        .from(admins)
        .where(eq(admins.phoneNumber, user.phoneNumber))
        .limit(1);
      if (existingAdmin) {
        res.status(409).json({
          success: false,
          error: "An administrator account already exists for this member.",
        });
        return;
      }
      ...
      await db
        .update(reservations)
        .set({ adminId: newAdmin.id, userId: null })
        .where(eq(reservations.userId, user.id));
      await db
        .update(reservationSeries)
        .set({ adminId: newAdmin.id, userId: null })
        .where(eq(reservationSeries.userId, user.id));
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (
        ...
        res.status(403).json({
          success: false,
          error: "This account is permanently protected and cannot be demoted.",
        });
        return;
      }
      ...
      await db
        .update(reservations)
        .set({ userId: targetUser.id, adminId: null })
        .where(eq(reservations.adminId, admin.id));
      await db
        .update(reservationSeries)
        .set({ userId: targetUser.id, adminId: null })
        .where(eq(reservationSeries.adminId, admin.id));
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!normalizedEmail || !isValidEmail(normalizedEmail)) {
        res.status(400).json({
          success: false,
          error: "Valid administrator email address is required.",
        });
        return;
      }
      if (!phoneNumber) {
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!target) {
        res
          .status(404)
          .json({ success: false, error: "Admin account not found." });
        return;
      }
      ...
      if (
        ...
        res.status(403).json({
          success: false,
          error: "This account is permanently protected and cannot be deleted.",
        });
        return;
      }
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      const body = req.body || {};
      const rawActive =
        body.maxActiveReservations ?? body.max_active_reservations;
      const rawPerDay =
        body.maxReservationsPerDay ?? body.max_reservations_per_day;
      const rawDuration = body.maxDurationHours ?? body.max_duration_hours;
      const rawConcurrent =
        body.maxConcurrentPerType ?? body.max_concurrent_per_type;
      const rawSeries =
        body.maxSeriesOccurrences ?? body.max_series_occurrences;
      ...
      const newActive = parseNum(
        rawActive,
        existing?.maxActiveReservations ?? 5,
      );
      const newPerDay = parseNum(
        rawPerDay,
        existing?.maxReservationsPerDay ?? 5,
      );
      const newDuration = parseNum(
        rawDuration,
        existing?.maxDurationHours ?? 5,
      );
      const newConcurrent = parseNum(
        rawConcurrent,
        existing?.maxConcurrentPerType ?? 2,
      );
      const newSeries = parseNum(
        rawSeries,
        existing?.maxSeriesOccurrences ?? 8,
      );
      const newSubmissions = parseNum(
        rawSubmissions,
        existing?.maxSubmissionsPerHour ?? 10,
      );
      const newBypass =
        rawBypass !== undefined
          ? Boolean(rawBypass)
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const result = await db.execute(
        sql`SELECT id FROM payment_settings LIMIT 1`,
      );
