...
export const instruments = pgTable(
  ...
  (table) => [
    check(
      "booking_mode_check",
      sql`booking_mode = ANY (ARRAY['manual'::text, 'instant'::text])`,
    ),
  ],
);
...
export const reservationSeries = pgTable(
  ...
  (table) => [
    ...
    check(
      "pattern_type_check",
      sql`pattern_type = ANY (ARRAY['weekly'::text, 'custom'::text])`,
    ),
  ],
);
...
export const reservations = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`reservation_type = ANY (ARRAY['in_church'::text, 'outside_church'::text])`,
    ),
    check(
      "status_check",
      sql`status = ANY (ARRAY['pending'::text, 'approved'::text, 'rejected'::text, 'auto_rejected'::text, 'cancelled'::text, 'ongoing'::text, 'completed'::text, 'expired'::text])`,
    ),
  ],
);
...
export const trustedStatusAuditLog = pgTable(
  ...
  (table) => [
    ...
    check(
      "audit_action_check",
      sql`action = ANY (ARRAY['granted'::text, 'revoked'::text])`,
    ),
  ],
);
