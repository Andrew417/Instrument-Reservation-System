...
async function runTests() {
  ...
  try {
    ...
    console.log(
      `- Manual Instrument: ${manualPiano.name} (${manualPiano.id}) [mode: ${manualPiano.bookingMode}]`,
    );
    console.log(
      `- Instant Instrument: ${instantGuitar.name} (${instantGuitar.id}) [mode: ${instantGuitar.bookingMode}]`,
    );
    console.log(
      `- Trusted User: ${trustedUser.name} (${trustedUser.id}) [isTrusted: ${trustedUser.isTrusted}]`,
    );
    console.log(
    ...
    console.log(
      `Assertion Test 1 (status === 'approved'):`,
      test1Result.reservation.status === "approved" ? "PASSED ✅" : "FAILED ❌",
    );
    console.log();
    ...
    console.log("------------------------------------------------------------");
    console.log(
      "TEST 2: Regular user over max_active_reservations booking Instant instrument",
    );
    console.log("------------------------------------------------------------");
    // First, fill up regularUser's active reservations to max_active_reservations (5)
    console.log(
      "Creating 5 distinct active reservations to reach max limit (5)...",
    );
    for (let i = 1; i <= 5; i++) {
    ...
    console.log(
      `Assertion Test 2 (status === 'pending'):`,
      test2Result.reservation.status === "pending" ? "PASSED ✅" : "FAILED ❌",
    );
    console.log();
    ...
    console.log(
      `User A Submission Status: ${firstSubmission.reservation.status} (ID: ${firstSubmission.reservation.id})`,
    );
    ...
    console.log(
      `Assertion Test 3 (Second overlapping booking blocked outright):`,
      secondSubmissionBlocked ? "PASSED ✅" : "FAILED ❌",
    );
    console.log();
    ...
    console.log(
      ...
      seriesBlocked && hasBothOccurrencesInError ? "PASSED ✅" : "FAILED ❌",
    );
    console.log();
